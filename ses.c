                     ...
                     int buf_size, int *count)
{
  ...
  ret = sg_ll_receive_diag(sg_fd, 1 /* pcv */, page_code, buf,
                           buf_size, 1, 0);
#ifdef DEBUG
  perr("sg_read_ses_page: page_code: %d\n", page_code);
#endif
  if (0 == ret) {
    *count = (buf[2] << 8) + buf[3] + 4;
#ifdef DEBUG
  perr("sg_read_ses_page: count: %d\n", *count);
#endif
                    ...
                    int *page_two_size)
{
  ...
  rc = sg_read_ses_page(sg_fd, PAGE_ZERO, pages->page_zero, MAX_SES_PAGE_SIZE, &size);
  if (0 != rc) {
    return rc;
  }

  rc = sg_read_ses_page(sg_fd, PAGE_ONE, pages->page_one, MAX_SES_PAGE_SIZE, &size);
  if (0 != rc) {
    return rc;
  }

  rc = sg_read_ses_page(sg_fd, PAGE_TWO, pages->page_two, MAX_SES_PAGE_SIZE, &size);
  if (0 != rc) {
    return rc;
  }
  if (page_two_size)
    *page_two_size = size;
  // TODO: exclude temporary
  //rc = sg_read_ses_page(sg_fd, PAGE_FIVE, pages->page_five, MAX_SES_PAGE_SIZE, &size);
  //if (0 != rc) {
  //  return rc;
  //}

  rc = sg_read_ses_page(sg_fd, PAGE_SEVEN, pages->page_seven, MAX_SES_PAGE_SIZE, &size);
  if (0 != rc) {
    return rc;
  }

  rc = sg_read_ses_page(sg_fd, PAGE_A, pages->page_a, MAX_SES_PAGE_SIZE, &size);
  if (0 != rc) {
    return rc;
  }
  ...
  struct ses_status_info *ses_info)
{
  ...
  for (i = 0; i < ses_info->temp_count; i ++)
    print_temperature_sensor(ses_info->temp_sensors + i, 1);
  for (i = 0; i < ses_info->vol_count; i ++)
    print_volatage_sensor(ses_info->vol_sensors + i, 1);
  for (i = 0; i < ses_info->curr_count; i ++)
    print_current_sensor(ses_info->curr_sensors + i, 1);
...
void verify_additional_element_eip_pcie (
  unsigned char *additional_element)
{
  // assert EIP == 1
  assert((additional_element[0] & 0x10) == 0x10);
...
#ifdef DEBUG
  perr("verify_additional_element_eip_pcie: additional_element: %d\n", additional_element[0]);
#endif  /* DEBUG */
  // assert protocol identifier says PCIe
  assert((additional_element[0] & 0x0f) == 0xb);
}
