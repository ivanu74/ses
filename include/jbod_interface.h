...
#define INQUIRY_SERIAL_INITIAL_LEN 252
#define INQUIRY_SERIAL_LEN 36

/*
A JBOD type, eg yadro
*/
struct jbod_profile  {
...
struct jbod_serial  {
  char serial[INQUIRY_SERIAL_LEN + 1];
};
/*
...
typedef struct jbod_interface {
  ...
  /* print jbod profile */
  void (*print_profile) (const char *devname, char* buff, size_t* size);
  ...
  /* print jbod sas addresses */
  void (*print_sasaddress_info) (int sg_fd, char* buff, size_t* size);
  /* print jbod serial number */
  void (*print_serial) (const char *devname, char* buff, size_t* size);
} jbod_interface_t;

extern struct jbod_interface yadro;

/* all supported JBODs: yadro, etc. */
...
extern void jbod_print_profile(const char *devname, char* buff, size_t* size);

extern struct jbod_short_profile jbod_get_short_profile (char *devname);
extern void jbod_print_pwm(int sg_fd);
extern void jbod_print_cfm(int sg_fd);

extern void jbod_print_sasaddress_info(int sg_fd, char* buff, size_t* size);
