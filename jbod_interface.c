...
#define SAS_ADDRESS_EMPTY    "0000000000000000"

struct jbod_profile jbod_library[] = {
  {"YADRO   ", "SXP 48x12G      ", "", "", &yadro, "YADRO"},
  {"YADRO", "TATLIN", "", "", &yadro, "YADRO"},
};
...
#ifdef DEBUG
    perr("device: %s specific: %s\n", devname, profile->specific);
#endif
    profile->specific[0] = '\0';
  }
#ifdef DEBUG
  char bb[1024];
  dStrHexStr(&buff[36], INQUIRY_SPECIFIC_LEN, "        ", 0, sizeof(bb), bb);
  perr("device: %s specific: %s\n", devname, bb);
#endif
...
#ifdef DEBUG
  perr("device: %s vendor: %s len: %ld\n", devname, profile->vendor, strlen(profile->vendor));
  perr("device: %s product: %s len: %ld\n", devname, profile->product, strlen(profile->product));
#endif
...
int jbod_hdd_led_control (int sg_fd, int slot_id, int op)
{
  ...
  int page_two_size;
#ifdef DEBUG
  perr("jbod_hdd_led_control: slot_id: %d op: %d\n", slot_id, op);
#endif
...
#ifdef DEBUG
      char bb[16];
      dStrHexStr(&pages.page_two[ses_status.slots[slot_id].page_two_offset], 16, " ", 0, sizeof(bb), bb);
      perr("jbod_hdd_led_control: additional elements: %s\n", bb);
#endif
...
void jbod_print_profile(const char *devname, char* buff, size_t* size)
{
  struct jbod_profile *profile = NULL;
  profile = extract_profile(devname);
  if (!profile) { return;};
  if (*size <= sizeof(profile->vendor) +
               sizeof(profile->product) +
...
void jbod_print_sasaddress_info(int sg_fd, char* buff, size_t* size)
{
  int rc;
  struct ses_status_info ses_info = {};
  //size_t n = 0;
  ...
  if (0 == rc) {
    IF_PRINT_NONE_JSON {
      for (int i = 0; i < ses_info.slot_count; ++i) {
        if (strcmp(ses_info.slots[i].sas_addr_str, SAS_ADDRESS_EMPTY) != 0) {
          total += sprintf(buff + total, "%d\n", ses_info.slots[i].slot);
          total += sprintf(buff + total, "%s\n", ses_info.slots[i].sas_addr_str);
          total += sprintf(buff + total, "%s\n", ses_info.slots[i].attached_sas_addr_str);
        }
#ifdef DEBUG
        perr("sas address: %s\n%s\n", ses_info.slots[i].sas_addr_str, SAS_ADDRESS_EMPTY);
#endif
      }
    }
    *size = total;
...
void jbod_print_power_reading(int sg_fd)
{
 
}

void jbod_print_event_log(int sg_fd)
{
}

void jbod_print_pwm(int sg_fd)
{
}

void jbod_print_cfm(int sg_fd)
{
}

struct jbod_serial *extract_serial(const char *devname)
{
  struct jbod_serial *serial;
  char serial_buff[INQUIRY_SERIAL_INITIAL_LEN];
  int sg_fd;
  ...
  if (sg_ll_inquiry(sg_fd, 0, 1, VPD_UNIT_SERIAL_NUM, serial_buff, sizeof(serial_buff), 0, 0)) {
#ifdef DEBUG
    perr("Cannot inquiry 0x80 %s\n", devname);
#endif
    goto done;
  }
  serial = (struct jbod_serial *)calloc(1, sizeof(struct jbod_serial));
  if (serial == NULL) {
    goto done;
  }
  memcpy(serial->serial, &serial_buff[4], INQUIRY_SERIAL_LEN);

#ifdef DEBUG
    perr("device: %s serial: %s\n", devname, serial->serial);
#endif
...
extern void jbod_print_serial(const char *devname, char* buff, size_t* size)
{
  struct jbod_serial *serial = NULL;
  serial = extract_serial(devname);
  if (serial) {
    IF_PRINT_NONE_JSON {
    *size = sprintf(buff, "%s\n", serial->serial);
    }
...
struct jbod_interface yadro = {
  jbod_print_enclosure_info,
  jbod_print_temperature_reading,
  jbod_print_voltage_reading,
  jbod_print_current_reading,
  jbod_print_power_reading,
  jbod_print_all_sensor_reading,
  jbod_print_hdd_info,
  jbod_hdd_power_control,
  jbod_hdd_led_control,
  jbod_print_fan_info,
  jbod_control_fan_pwm,
  jbod_set_asset_tag,
  jbod_set_asset_tag_by_name,
  jbod_print_asset_tag,
  jbod_power_cycle_enclosure,
  jbod_print_gpio,
  jbod_print_event_log,
  jbod_print_event_status,
  jbod_print_sys_led,
  jbod_control_sys_led,
  jbod_show_config,
  jbod_config_power_window,
  jbod_config_hdd_temp_interval,
  jbod_config_fan_profile,
  jbod_identify_enclosure,
  jbod_print_phyerr,
  jbod_reset_phyerr,
  jbod_print_profile,
  jbod_get_short_profile,
  jbod_print_pwm,
  jbod_print_cfm,
  jbod_print_sasaddress_info,
  jbod_print_serial
};
